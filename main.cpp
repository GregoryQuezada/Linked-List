...
void TestConstructor()
{
	...
	cout << "Initial list: " << endl;
	dataInt.PrintForward();

	LinkedList<int> data2 = dataInt;

	if (data2 == dataInt)
		cout << "They are equal.";
	data2.PrintForward();

	LinkedList<string> data3;
	string test = "great benefit linked is the ability to easily into the";
	stringstream x(test);
	string tmp;
	while (getline(x, tmp, ' '))
	...
	cout << "Initial list: " << endl;
	data3.PrintForward();
	cout << "Node count: " << data3.NodeCount() << endl;

	cout << "\nInserting words into the list with InsertAt()..." << endl;

	data3.InsertAt("One", 0);
	data3.InsertAt("of", 3);
	data3.InsertAt("lists", 5);
	data3.InsertAt("insert", 10);
	data3.InsertAt("nodes", 11);
	data3.InsertAt("list.", 15);

	data3.PrintForward();
	cout << "Node count: " << data3.NodeCount() << endl;

	LinkedList<string> data4 = data3;
	if (data4 == data3)
	{
		cout << "Zybooks should be working.";
		cout << "\n\n";
		data4.PrintForward();
		cout << "Node count: " << data4.NodeCount() << endl; 
	}
}
